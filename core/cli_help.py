...
def print_cli_help():
    ...
    opt_table.add_row("--server [PORT]", "Companion Relay Server: launches Go background engine with UDP discovery for Hwaran Android app (default: 53318).")
    opt_table.add_row("--resolve-link <URL>", "Link Resolver: fast headless JSON analyzer reporting valid status, site engine, category, and scope flags.")
    ...
    cmd_table.add_row("server", "Remote Relay", "Start high-performance companion server for Hwaran (Android) and remote ingestion.")
    ...
    ex_text.append("  # Launch the companion server for Hwaran phone ingestion:\n", style="dim white")
    ex_text.append("  zine --server\n\n", style="bold cyan")
    ex_text.append("  # Extract & save series metadata card and cover art only (--meta):\n", style="dim white")
    ex_text.append("  zine \"https://chikari.moe/novels/my-novel-slug\" --meta\n\n", style="bold cyan")
    ex_text.append("  # Headlessly analyze and inspect link architecture & metadata:\n", style="dim white")
    ex_text.append("  zine --resolve-link \"https://youtube.com/@channel/videos\"\n\n", style="bold cyan")
    ...
    ex_text.append("  # Verify all system binaries, GPU, and permissions:\n", style="dim white")
