...
# No ColorHandler added to root_logger anymore to avoid terminal clutter
...
class BaseScraper:
    ...
    IMAGE_DELAY = 0.3  # Balanced delay for speed and accuracy
    ...
    def get_session(cls, domain):
        if not hasattr(cls._tls, "session"):
            s = requests.Session()
            s.headers.update(HEADERS)
            s.headers["Referer"] = f"https://{domain}/"
    ...
    def download_image(self, src: str, path: Path, referer: str = None) -> int:
        ...
        for attempt in range(2):
            try:
                ...
                r = session.get(src, stream=True, timeout=(8, 15), headers=headers)
                ...
                try:
                    ...
                    if actual_format:
                        fmt_to_ext = {"jpeg": ".jpg", "png": ".png", "webp": ".webp", "avif": ".avif", "gif": ".gif", "bmp": ".bmp"}
                        correct_ext = fmt_to_ext.get(actual_format)
                        if correct_ext and path.suffix.lower() != correct_ext:
                            new_path = path.with_suffix(correct_ext)
                            path.rename(new_path)
                except Exception as e:
                    logging.debug(f"Image format check failed for {path}: {e}")
