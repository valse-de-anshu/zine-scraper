...
class BaseScraper:
    ...
    def download_cover(self, folder: Path):
        ...
        raw_temp = temp_root / f"manhwaus_cover_{int(time.time() * 1000)}.tmp"
                    ...
                    if raw_temp.exists() and raw_temp.stat().st_size > 500:
                        ...
                        try:
                            with Image.open(raw_temp) as im:
                                # Convert heavy/mislabelled images (e.g. huge PNG disguised under .jpg URL) to clean JPEG
                                # Ensures universal thumbnail rendering across all desktop file managers (Dolphin, Nautilus, etc.)
                            ...
                            ext = ".png" if real_format == "PNG" else (".webp" if real_format == "WEBP" else ".jpg")
                            shutil.copy2(raw_temp, folder / f"cover{ext}")
    ...
    def process_chapter_multi(self, img_urls: List[str], folder: Path, ch_num: str, ch_url: str, live=None, stats_callback=None) -> dict:
        ...
        temp_dir = PathAuthority().get_temp_root() / f"manhwaus_ch_{safe_num}_{int(time.time() * 1000)}"
