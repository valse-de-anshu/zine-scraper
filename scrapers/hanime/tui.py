"""
scrapers/hanime/tui.py
------------------------
Hanime TUI — Hentai-category flow.
...
from .scraper import HanimeScraper
from .workflow import run_workflow

logger = logging.getLogger(__name__)


def handle_hanime_tui(
    url: str,
    ...
    with active_status("[info]Loading Hanime metadata...[/info]", spinner="dots"):
        try:
    ...
    if not metadata or videos is None:
        console.print("[error]Could not retrieve Hanime metadata.[/error]")
        time.sleep(2)
        return
    ...
    if batch_path is not None:
        ...
        if cfg.exists():
            try:
                ...
                pass
        if is_vacuum:
            target_root = lib / "Vacuum" / "Hentai" / "Hanime"
        else:
