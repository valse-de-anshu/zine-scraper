"""
scrapers/hanime/workflow.py
-----------------------------
Hanime download orchestrator.
...
def run_workflow(
    ...
    """
    Main download orchestrator for Hanime.

    target_root  → e.g. .../Zine/Vacuum/hanime  or .../Zine/Quick grab/hanime
    creator_root → e.g. .../Vacuum/hanime/miulio  (resolved collision-free)
    ...
    if is_vacuum:
        try:
            ...
            logger.error(f"Failed to save Hanime metadata/cover: {e}")
    ...
    for idx, video in enumerate(videos, 1):
        ...
        baking_spinner = Spinner("dots", style="menu")
        ...
        def render_video_tree() -> Tree:
            ...
            if not progress_data["done"]:
                ...
                if progress_data.get("baking") or is_100_percent:
                    ...
                    baking_spinner = f"[cyan]{frame}[/cyan] [unselected]{status_text}[/unselected]"
                    res_branch.add(baking_spinner)
                else:
                    if is_small_file:
        ...
        def yt_dlp_hook(d):
            ...
            elif d.get("baking") or d["status"] == "baking":
                if filename and filename in completed_files:
                    completed_files[filename]["downloaded"] = completed_files[filename]["total"]
                    progress_data["downloaded_bytes"] = sum(f["downloaded"] for f in completed_files.values())
                progress_data["status"] = "Almost done with baking..."
                progress_data["baking"] = True
        ...
        try:
            ...
            while True:
                ...
                with Live(render_video_tree(), console=console, refresh_per_second=10, transient=True) as live:
                    ...
                    try:
                        success = scraper.engine.download_hanime_video(
                            vid_url,
                            vid_sub_folder,
                            active_hook,
                            quality=quality,
