...
------------------------
Ohentai TUI — Hentai-category flow.
...
from .scraper import OhentaiScraper
...
def handle_ohentai_tui(
    url: str,
    tracker: HistoryLayer,
    library_root: Path,
    storage_layer: StorageLayer,
    scraper: OhentaiScraper,
    is_batch_mode: bool = False,
    batch_path: Optional[Path] = None,
):
    """
    TUI flow for Ohentai.
    ...
    with active_status("[info]Loading Ohentai metadata...[/info]", spinner="dots"):
        try:
    ...
    if not metadata or videos is None:
        console.print("[error]Could not retrieve Ohentai metadata.[/error]")
        time.sleep(2)
        return
