"""
scrapers/ohentai/workflow.py
-----------------------------
Ohentai download orchestrator.
...
def run_workflow(
    ...
    """
    Main download orchestrator for Ohentai.

    target_root  → e.g. .../Zine/Vacuum/ohentai  or .../Zine/Quick grab/ohentai
    creator_root → e.g. .../Vacuum/ohentai/miulio  (resolved collision-free)
    ...
    if is_vacuum:
        try:
            ...
            logger.error(f"Failed to save Ohentai metadata/cover: {e}")
            ...
            while True:
                ...
                with Live(render_video_tree(), console=console, refresh_per_second=10, transient=True) as live:
                    ...
                    try:
                        ...
                        success = engine.download_ohentai_video(
                            url=vid_url,
