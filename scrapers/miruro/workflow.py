...
def run_workflow(url: str, tracker: Any, location_manager: Any, scraper: Any,
        ...
        if choice == "single":
            if is_single_ep_url:
                ...
                if ep_match:
                    ...
                    target_videos = [v for v in videos if ep_pattern.search(v.get("url", "")) or str(v.get("id", "")).endswith(f"_ep{ep_num}")]
                    if not target_videos:
                        try:
                            raw_list = scraper.get_metadata_and_videos()[1]
                            target_videos = [v for v in raw_list if ep_pattern.search(v.get("url", "")) or str(v.get("id", "")).endswith(f"_ep{ep_num}")]
                        except Exception:
                            target_videos = []
                    if target_videos:
    ...
    if is_single_episode and not is_batch:
        ...
        payload = ZineMetadataPayload(
            title=title,
            type="Series",
            alt_title=metadata.get("Romaji Title", "") or metadata.get("Native Title", ""),
            description=metadata.get("Description", ""),
            status=metadata.get("Status", ""),
            rating=str(metadata.get("Score", "") or metadata.get("Rating", "")),
            tags=genres,
            year=str(metadata.get("Season Year", "") or metadata.get("Year", "")),
            url=url
        )
