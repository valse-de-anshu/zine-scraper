...
def run_workflow(url: str, tracker: Any, location_manager: Any, scraper: Any, batch_path: Optional[Path] = None, is_batch: bool = False):
    ...
    status_str = getattr(scraper, "status", "") or next((part for part in target_path.parts if part.lower() in ["ongoing", "completed", "complete"]), "")
    ...
    # Detect any pre-existing cover regardless of extension (cover.webp, cover.png, etc.)
    existing_cover_files = list(folder.glob("cover.*")) if folder.exists() else []
    cover_exists = bool(existing_cover_files)
    ...
    if "Quick grab" in target_path.parts:
        ...
        if not cover_exists:
            try:
                ...
                existing_cover_files = list(folder.glob("cover.*"))
                cover_exists = bool(existing_cover_files)
            except Exception:
                pass
    ...
    for ch_num, link in to_process:
        ...
        # Page-count stats dict — matches toon engine stats_callback format
        page_data = {
            "total":      0,
            "downloaded": 0,
            "retry":      0,
        ...
        for attempt in range(1, 4):
            ...
            # Live has exited NORMALLY here — cursor is clean below last rendered position
            _LIVE_INSTANCE = None
            ...
            if _chapter_error[0] is not None:
                ...
                # process_chapter returned False/None without exception — no retry needed
                break
