"""
scrapers/hstream/workflow.py
-----------------------------
Hstream download orchestrator.
...
def run_workflow(
    ...
    """
    Main download orchestrator for Hstream.

    target_root  → e.g. .../Zine/Vacuum/hstream  or .../Zine/Quick grab/hstream
    creator_root → e.g. .../Vacuum/hstream/miulio  (resolved collision-free)
    ...
    if is_vacuum:
        try:
            ...
            logger.error(f"Failed to save Hstream metadata/cover: {e}")
            ...
            while True:
                ...
                with Live(render_video_tree(), console=console, refresh_per_second=10, transient=True) as live:
                    ...
                    try:
                        ...
                        success = engine.download_hstream_video(
                            url=vid_url,
