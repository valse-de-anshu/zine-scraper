...
------------------------
Hstream TUI — Hentai-category flow.
...
from .scraper import HstreamScraper
from .workflow import run_workflow

logger = logging.getLogger(__name__)


def handle_hstream_tui(
    url: str,
    tracker: HistoryLayer,
    library_root: Path,
    storage_layer: StorageLayer,
    scraper: HstreamScraper,
    is_batch_mode: bool = False,
    batch_path: Optional[Path] = None,
):
    """
    TUI flow for Hstream.
    ...
    with active_status("[info]Loading Hstream metadata...[/info]", spinner="dots"):
        try:
    ...
    if not metadata or videos is None:
        console.print("[error]Could not retrieve Hstream metadata.[/error]")
        time.sleep(2)
        return
