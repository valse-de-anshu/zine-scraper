"""
scrapers/manga18fx/location.py
------------------------------
Save-location resolver for Manga18fx.
Controls SFW/NSFW classification, Ongoing/Completed status, and directory routing.
"""
import sys
import time
...
def get_save_path(
    ...
    toon_name = getattr(scraper, "title", None)
    if not toon_name:
    ...
    # Detect NSFW from metadata if present
    genres = [g.lower() for g in getattr(scraper, "genres", [])]
    is_nsfw_series = any(g in ["mature", "adult", "erotica", "smut", "ecchi", "hentai", "18+"] for g in genres)
    ...
    while True:
        if state == 0:
            draw_header()
            options = [("NSFW", "NSFW"), ("SFW", "SFW"), ("Back", "BACK")] if default_type == "NSFW" else [("SFW", "SFW"), ("NSFW", "NSFW"), ("Back", "BACK")]
            choice = Selector(options, "Type").select()
            if choice == "BACK":
                return None
            if choice == "=":
