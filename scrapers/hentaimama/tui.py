...
------------------------
Hentaimama TUI — Hentai-category flow.
...
from .scraper import HentaimamaScraper
from .workflow import run_workflow

logger = logging.getLogger(__name__)


def handle_hentaimama_tui(
    url: str,
    ...
    with active_status("[info]Loading Hentaimama metadata...[/info]", spinner="dots"):
        try:
    ...
    if not metadata or videos is None:
        console.print("[error]Could not retrieve Hentaimama metadata.[/error]")
        time.sleep(2)
        return
    ...
    if batch_path is not None:
        ...
        if cfg.exists():
            try:
                ...
                pass
        if is_vacuum:
            target_root = lib / "Vacuum" / "Hentai" / "Hentaimama"
        else:
