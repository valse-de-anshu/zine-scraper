"""
scrapers/light_novel/lightnovelworld/location.py
-------------------------------------------------
Save-path resolver for LightNovelWorld novels.
Routes to Vacuum (full novel) or Quick grab (single chapter).
"""
...
def get_save_path(url: str, scraper: Any, is_batch: bool, batch_path: Optional[Path],
    ...
    # If Quick grab, we just skip categorization for now or keep it consistent?
    # Actually, the user wants <Vacuum>/Light Novel/<site>/<tag>/<status>.
    # Quick grab will use default_root = <Quick grab>/Light Novel/<site>.
    ...
    # Fallback if non-interactive but not batch (or if skipped)
