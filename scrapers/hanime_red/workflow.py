"""
scrapers/hanime_red/workflow.py
-----------------------------
HanimeRed download orchestrator.
...
def run_workflow(
    ...
    """
    Main download orchestrator for HanimeRed.

    target_root  → e.g. .../Zine/Vacuum/hanime_red  or .../Zine/Quick grab/hanime_red
    creator_root → e.g. .../Vacuum/hanime_red/miulio  (resolved collision-free)
    ...
    if is_vacuum:
        try:
            ...
            logger.error(f"Failed to save HanimeRed metadata/cover: {e}")
            ...
            while True:
                ...
                with Live(render_video_tree(), console=console, refresh_per_second=10, transient=True) as live:
                    ...
                    try:
                        ...
                        success = engine.download_hanime_red_video(
                            url=vid_url,
