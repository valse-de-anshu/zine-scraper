...
def render_progress_tree(state: dict) -> Tree:
    """Construct and return the visual Tree for Live status updates during Facebook downloads."""
    tree = Tree(
    ...
    if state["status"] == "extracting":
        ...
        tree.add(f"[{ball_style}]●[/{ball_style}] Intercepting and extracting Facebook media items...")
        return tree

    tree.add(align_header("Total Items", f"[info]{state['pins_total']}[/info]"))
