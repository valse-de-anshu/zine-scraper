...
def _find_existing_series_folder(anikoto_root: Path, title: str) -> Optional[Path]:
    """
    ...
    different anikoto mirror (e.g. anikototv.to vs anikoto.me).
    ...
    target = norm(title)
    if anikoto_root.exists():
        for d in anikoto_root.iterdir():
...
def _fetch_hls_qualities(master_url: str, headers: dict) -> list:
    ...
    try:
        ...
        logger.debug(f"[Anikoto] Could not fetch HLS qualities: {e}")
        return []
...
def _download_subtitles(subtitles: list, video_path: Path, headers: dict):
    ...
    for sub in subtitles:
        ...
        try:
            ...
            logger.info(f"[Anikoto] Saved subtitle: {sub_path.name}")
        except Exception as e:
            logger.warning(f"[Anikoto] Failed to download subtitle '{label}': {e}")
...
def run_workflow(url: str, tracker: Any, location_manager: Any, scraper: Any,
    ...
    if is_single_episode and not is_batch:
        ...
        existing = _find_existing_series_folder(anikoto_root, title)
        series_root = existing if existing else (anikoto_root / safe_title)
        ...
        try:
            ...
            custom_metadata = {
                ...
                "source": metadata.get("Source", "Anikoto"),
    ...
    if not is_single_episode:
        ...
        tree.add(_align("Source", f"[info]{metadata.get('Source', 'Anikoto')}[/info]"))
        console.print(tree)
        console.print("")
    ...
    for idx, video in enumerate(videos, 1):
        ...
        with Live(render_video_tree(), console=console, refresh_per_second=12,
            ...
            anikoto_domains.insert(0, original_netloc)

            domain_success = False
            for target_domain in anikoto_domains:
                if domain_success:
                    break
                ...
                # Create a modified video dict with the target domain
                alt_vid_url = vid_url.replace(original_netloc, target_domain)
                alt_video = dict(video)
                alt_video["url"] = alt_vid_url

                if hasattr(scraper, "resolve_episode_stream"):
                    try:
                        stream_info = scraper.resolve_episode_stream(alt_video)
                        if stream_info and stream_info.get("m3u8_url"):
                            raw_stream_url = stream_info["m3u8_url"]
                            referer = stream_info.get("referer")
                    except Exception as e:
                        console.print(f"[warning]Stream resolve error on {target_domain}: {e}[/warning]")
