"""
scrapers/light_novel/novelphoenix/scraper.py
--------------------------------------------
NovelPhoenix.com scraper.

URLs supported:
  - Novel page:   https://novelphoenix.com/novel/<slug>
  - Chapter page: https://novelphoenix.com/novel/<slug>/chapter-<num>
"""
...
from .engine import NPBaseEngine

logger = logging.getLogger("NovelPhoenix")


class NovelPhoenixScraper(NPBaseEngine):
    scraper_type = "novel"
    ...
    def __init__(self, url: str):
        ...
        match = re.search(r"(novelphoenix\.com/novel/[^/]+)", url)
        if match:
    ...
    def get_title_and_chapters(self):
        ...
        # Extract title
        title_el = soup.select_one("h1, .novel-title, .title")
        ...
        author_el = soup.select_one(".author a, .meta-item:-soup-contains('Author') a")
        ...
        if cover_el:
            ...
            if src.startswith("//"):
                ...
                src = f"https://novelphoenix.com{src}"
            self.cover_url = src
        else:
            self.cover_url = ""
    ...
    def _fetch_all_chapters(self):
        ...
        while page <= max_pages:
            ...
            for a in links:
                href = a["href"]
                if "/chapter-" in href:
                    full_url = href if href.startswith("http") else f"https://novelphoenix.com{href}"
                    if full_url in seen:
                        continue
                    seen.add(full_url)
    ...
    def process_chapter(self, ch_url: str, folder: Path, ch_num: str,
        ...
        try:
            ...
            logger.error(f"NovelPhoenix chapter {ch_num} failed: {e}")
            if stats_callback:
