"""
scrapers/light_novel/chikari/verification.py
--------------------------------------------
Chapter verification for Chikari.
"""
...
def verify_chapters(folder: Path, chapters: List[Tuple[str, str]], tracker: Any, url: str) -> Tuple[List[str], List[Tuple[str, str]]]:
    ...
    for num, link in chapters:
        ...
        alt_dir = folder / f"Chapter {num}"

        is_in_history = tracker.is_downloaded(url, num)
        has_file = ch_file.exists() or alt_file.exists() or (alt_dir.exists() and any(alt_dir.iterdir()))
