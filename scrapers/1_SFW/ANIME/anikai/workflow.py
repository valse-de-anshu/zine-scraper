"""
scrapers/anikai/workflow.py
────────────────────────────
Full download orchestrator for Anikai. Mirrors anitaku/workflow.py but is
...
def _find_existing_series_folder(anikai_root: Path, title: str) -> Optional[Path]:
    """Prevents duplicate folders when re-scraping the same series from a different mirror."""
    def norm(s: str) -> str:
        return re.sub(r'[^a-z0-9]', '', s.lower())

    target = norm(title)
    if anikai_root.exists():
        for d in anikai_root.iterdir():
...
def _download_subtitles(subtitles: list, video_path: Path, headers: dict):
    ...
    for sub in subtitles:
        ...
        try:
            ...
            logger.warning(f"[Anikai] Failed to download subtitle '{label}': {e}")
...
def _fetch_hls_qualities(master_url: str, headers: dict) -> list:
    ...
    try:
        ...
        logger.debug(f"[Anikai] Could not fetch HLS qualities: {e}")
        return []
...
def run_workflow(
    ...
    if is_single_episode and not is_batch:
        ...
        existing = _find_existing_series_folder(anikai_root, title)
        series_root = existing if existing else (anikai_root / safe_title)
    ...
    if success_count > 0 or skipped_count == total:
        ...
        ui.print_alternative_anime_sources(title, current_site="AniKai")
