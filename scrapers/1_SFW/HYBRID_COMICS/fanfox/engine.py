...
class BaseScraper:
    ...
    def __init__(self, url: str):
        ...
        self.session.cookies.set("isAdult", "1", domain=".fanfox.net")
    ...
    def download_cover(self, folder: Path):
        ...
        raw_temp = temp_root / f"fanfox_cover_{int(time.time() * 1000)}.tmp"
    ...
    def process_chapter_multi(self, img_urls: List[str], folder: Path, ch_num: str, ch_url: str, live=None, stats_callback=None) -> dict:
        ...
        temp_dir = PathAuthority().get_temp_root() / f"fanfox_ch_{safe_num}_{int(time.time() * 1000)}"
