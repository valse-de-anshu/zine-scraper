    ...
    def resolve_episode_streams(self, episode_url: str) -> list:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    engine_logger.debug(f"[AnikaiEngine] Watch page fetch failed: {e}")
                    return []
        ...
        for embed_url, server_name in embed_entries:
            try:
                ...
                engine_logger.debug(f"[AnikaiEngine] Fast extraction error on {embed_url}: {e}")
                continue
        ...
        if not candidates:
            for embed_url, server_name in embed_entries:
                try:
                    ...
                    engine_logger.debug(f"[AnikaiEngine] Server {embed_url} failed: {e}")
                    continue
    ...
    def download_video(
        ...
        if target and ".m3u8" in target and not is_audio:
            try:
                ...
                import logging
                logging.getLogger(__name__).debug(f"[AnikaiEngine] Fast HLS download failed: {e}")
            if raw_stream_url:
                return False
