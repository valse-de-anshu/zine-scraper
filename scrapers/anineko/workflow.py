"""
scrapers/anineko/workflow.py
────────────────────────────
Full download orchestrator for AniNeko. Mirrors anitaku/workflow.py but is
...
def _find_existing_series_folder(anineko_root: Path, title: str) -> Optional[Path]:
    """Prevents duplicate folders when re-scraping the same series from a different mirror."""
    def norm(s: str) -> str:
        return re.sub(r'[^a-z0-9]', '', s.lower())

    target = norm(title)
    if anineko_root.exists():
        for d in anineko_root.iterdir():
...
def _download_subtitles(subtitles: list, video_path: Path, headers: dict):
    ...
    for sub in subtitles:
        ...
        try:
            ...
            logger.warning(f"[AniNeko] Failed to download subtitle '{label}': {e}")
...
def _fetch_hls_qualities(master_url: str, headers: dict) -> list:
    ...
    try:
        ...
        logger.debug(f"[AniNeko] Could not fetch HLS qualities: {e}")
        return []
...
def run_workflow(
    ...
    anineko_root = library_root / "Vacuum" / "Anime" / "anineko"
    ...
    if is_single_episode and not is_batch:
        ...
        existing = _find_existing_series_folder(anineko_root, title)
        series_root = existing if existing else (anineko_root / safe_title)
    ...
    if not is_single_episode:
        ...
        tree.add(f"{'Source':<18} : [info]AniNeko[/info]")
        console.print(tree)
        console.print("")
                ...
                if chosen_quality_url and raw_stream_url:
                    try:
                        ...
                        if ep_qualities:
                            ...
                            if chosen_h:
                                best = next(
                                    (q for q in ep_qualities if f"{chosen_h}p" == q["label"]),
                                    None
                                )
                                if best:
