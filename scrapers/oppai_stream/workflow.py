"""
scrapers/oppai_stream/workflow.py
-----------------------------
OppaiStream download orchestrator.
...
def run_workflow(
    ...
    """
    Main download orchestrator for OppaiStream.

    target_root  → e.g. .../Zine/Vacuum/oppai_stream  or .../Zine/Quick grab/oppai_stream
    creator_root → e.g. .../Vacuum/oppai_stream/miulio  (resolved collision-free)
    ...
    if is_vacuum:
        try:
            ...
            logger.error(f"Failed to save OppaiStream metadata/cover: {e}")
            ...
            while True:
                ...
                with Live(render_video_tree(), console=console, refresh_per_second=10, transient=True) as live:
                    ...
                    try:
                        ...
                        success = engine.download_oppai_stream_video(
                            url=vid_url,
