...
------------------------
OppaiStream TUI — Hentai-category flow.
...
from .scraper import OppaiStreamScraper
...
def handle_oppai_stream_tui(
    ...
    with active_status("[info]Loading OppaiStream metadata...[/info]", spinner="dots"):
        try:
    ...
    if not metadata or videos is None:
        console.print("[error]Could not retrieve OppaiStream metadata.[/error]")
        time.sleep(2)
        return
