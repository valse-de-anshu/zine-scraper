"""
scrapers/hentaihaven/workflow.py
-----------------------------
HentaiHaven download orchestrator.
...
def run_workflow(
    ...
    """
    Main download orchestrator for HentaiHaven.

    target_root  → e.g. .../Zine/Vacuum/hentaihaven  or .../Zine/Quick grab/hentaihaven
    creator_root → e.g. .../Vacuum/hentaihaven/miulio  (resolved collision-free)
        ...
        def render_video_tree() -> Tree:
            ...
            if not progress_data["done"]:
                ...
                if progress_data.get("baking") or is_100_percent:
                    ...
                    res_branch.add(f"[{ball_style}]●[/{ball_style}] {status}")
            else:
        ...
        try:
            ...
            while True:
                ...
                with Live(render_video_tree(), console=console, refresh_per_second=10, transient=True) as live:
                    ...
                    try:
                        success = scraper.engine.download_hentaihaven_video(
                            vid_url,
                            sub_folder,
                            active_hook,
                            quality=quality,
