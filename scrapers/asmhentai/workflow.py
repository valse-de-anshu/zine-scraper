...
def run_workflow(url: str, tracker: Any, location_manager: Any, scraper: Any, batch_path: Optional[Path] = None, is_batch: bool = False):
    ...
    # .zine metadata folder (skip for Quick grab)
    ...
    from rich.progress import Progress, TextColumn, TaskProgressColumn, BarColumn
    ...
    for ch_num, link in to_process:
        ...
        def render_chapter_tree() -> Tree:
            ...
            if not page_data["done"]:
                ...
                if page_data.get("status") == "baking":
                    frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
                    frame = frames[int(_time.time() * 10) % len(frames)]
        ...
        console.print(f"  [{res_color}]●[/{res_color}] [unselected]Gallery {ch_num}[/unselected]")
        completed_history.append(f"  [{res_color}]●[/{res_color}] [unselected]Gallery {ch_num}[/unselected]")
        time.sleep(CHAPTER_DELAY)

    if success_count > 0:
        console.print(f"\n[success]✦[/success] Done: {success_count}/{len(to_process)} galleries saved\n")
    else:
