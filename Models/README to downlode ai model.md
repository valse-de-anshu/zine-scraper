...
The `Models/` directory serves as the unified local storage hub for all offline AI models used across the Zine Scraper suite:
- **Voice-to-Text (STT) / Subtitle Generator** (`subs` command) powered by `faster-whisper`
- **Text-to-Speech (TTS) / Audiobook Voice Engine** (`breeze` / `tts` command) powered by `Breeze-TTS-2` (C++ / Vulkan GGUF) and `Qwen-TTS`

---

## 📁 Directory Structure Overview

When fully populated, your `Models/` folder can look like this:

```text
Models/
├── README to downlode ai model.md
│
├── faster-whisper-large-v3-turbo/     # [Voice-to-Text] Flagship Subtitle Model (~1.6 GB)
│   ├── config.json
│   ├── model.bin
...
│   └── vocabulary.json
│
├── breeze-tts-2-q8_0.gguf              # [Text-to-Speech] Breeze-TTS-2 8-bit Quantized GGUF (~3.2 GB)
├── breeze-tts-2-fp16.gguf              # [Text-to-Speech] Breeze-TTS-2 Full Precision (Optional, ~6.5 GB)
│
└── Breeze-TTS-2.cpp/                  # [TTS Engine] Prebuilt / Compiled C++ Vulkan Binaries
    └── build/
        └── bin/
...
## 🎧 Part 1: Voice-to-Text (STT) — `faster-whisper`
...
## 🗣️ Part 2: Text-to-Speech (TTS) — `Breeze-TTS-2` (GGUF / C++)
...
Place the `.gguf` weight file directly inside `Models/`:

#### Option A: Q8_0 Quantized (Recommended — ~3.2 GB, ~4GB VRAM)
```bash
# Using Hugging Face CLI:
huggingface-cli download MediaTek-Research/Breeze-TTS-2-GGUF breeze-tts-2-q8_0.gguf --local-dir Models

# OR using curl:
curl -L -o Models/breeze-tts-2-q8_0.gguf "https://huggingface.co/MediaTek-Research/Breeze-TTS-2-GGUF/resolve/main/breeze-tts-2-q8_0.gguf"
```

#### Option B: FP16 Unquantized (~6.5 GB, ~8GB VRAM)
```bash
curl -L -o Models/breeze-tts-2-fp16.gguf "https://huggingface.co/MediaTek-Research/Breeze-TTS-2-GGUF/resolve/main/breeze-tts-2-fp16.gguf"
```

### ⚡ 2. Build or Install `Breeze-TTS-2.cpp` Vulkan Engine

Clone and build the lightweight C++ Vulkan runtime inside `Models/`:
...
Once built, the binary will reside at `Models/Breeze-TTS-2.cpp/build/bin/breeze-cli` (or `build/breeze-cli`). Zine will automatically discover and utilize it without requiring manual configuration!
...
3. Configure model options under the respective sub-menus:
   - **Whisper AI Subtitles**:
     - *Subtitles Model Path*: `Models/faster-whisper-large-v3-turbo`
   ...
   - **Breeze TTS 2 (GGUF / C++)**:
     - *Model GGUF Path*: `Models/breeze-tts-2-q8_0.gguf`
     - *Binaries Directory*: `Models/Breeze-TTS-2.cpp/build`
